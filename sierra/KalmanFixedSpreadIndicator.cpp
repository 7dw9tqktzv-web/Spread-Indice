...
SCSFExport scsf_KalmanFixedSpreadIndicator(SCStudyInterfaceRef sc)
{
    ...
    // ========================================================================
    // Z-SCORE (toggle: ln(A/B) vs Kalman spread)
    // ========================================================================
    bool useKalmanZ = (InZScoreOnKalman.GetYesNo() != 0);
    float zScore = 0.0f;

    if (useKalmanZ)
    {
        // Z-score on Kalman spread (logY - alpha - beta*logX) -- accounts for real beta
    ...
    {
        // Z-score on ln(A/B) -- assumes beta=1, alpha cancels
...
SCSFExport scsf_KalmanFixedDollarSpread(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef InResetTime    = sc.Input[9];
    SCInputRef InInvert       = sc.Input[10];
    ...
    if (sc.SetDefaults)
    {
        ...
        InResetTime.Name = "Reset Time (HHMM CT)";
        InResetTime.SetInt(0);
        InResetTime.SetIntLimits(0, 2359);

        InInvert.Name = "Invert (Short Spread)";
        InInvert.SetYesNo(0);
    ...
    // ========================================================================
    // DOLLAR SPREAD (delta from reset time, line starts at 0)
    // ========================================================================
    ...
    int ResetHHMM  = InResetTime.GetInt();
    ...
    // Reference value: reset at specified time, or first bar if time=0
    ...
    if (ResetHHMM > 0)
    {
        int resetH = ResetHHMM / 100;
        int resetM = ResetHHMM % 100;
        int barTime = sc.BaseDateTimeIn[sc.Index].GetTimeInSeconds();
        int resetSec = resetH * 3600 + resetM * 60;
        int prevBarTime = (sc.Index > 0) ? sc.BaseDateTimeIn[sc.Index - 1].GetTimeInSeconds() : -1;

        // Reset when bar crosses the reset time
        if (prevBarTime < resetSec && barTime >= resetSec)
            RefValue = rawSpread;
    }

    float delta = rawSpread - RefValue;
    if (InInvert.GetYesNo())
        delta = -delta;
    DollarSpread[sc.Index] = delta;
    ZeroLine[sc.Index] = 0.0f;
}
